...
def export_to_csv(json_file_path="jobs_extracted.json", csv_file_path=None):
    """
    Xuất dữ liệu từ JSON sang CSV với các trường đã định nghĩa
    """
    
    # Tạo đường dẫn file nếu không được cung cấp
    if csv_file_path is None:
        result_folder = "../result"
        os.makedirs(result_folder, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_file_path = os.path.join(result_folder, f"rpa_jobs_analysis_{timestamp}.csv")
    ...
    try:
        ...
        try:
            ...
            return csv_file_path, excel_file
        except ImportError:
...
def find_latest_json_file():
    """Tìm file JSON mới nhất trong folder result"""
    result_folder = "../result"
    if not os.path.exists(result_folder):
        print(f"❌ Không tìm thấy folder {result_folder}")
        return None
    
    json_files = [f for f in os.listdir(result_folder) if f.startswith('jobs_extracted_') and f.endswith('.json')]
    if not json_files:
        print(f"❌ Không tìm thấy file JSON nào trong {result_folder}")
        return None
    
    # Sắp xếp theo thời gian tạo và lấy file mới nhất
    json_files.sort(reverse=True)
    latest_file = os.path.join(result_folder, json_files[0])
    print(f"📁 Sử dụng file JSON mới nhất: {json_files[0]}")
    return latest_file
...
if __name__ == "__main__":
    ...
    if json_file:
        ...
        print("❌ Không thể tìm file JSON để xuất CSV/Excel")
